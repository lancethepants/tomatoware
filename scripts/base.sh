...
if [ ! -f .certs_installed ]; then
	...
	c_rehash .
	touch $SRC/curl/curl-7.50.0/.certs_installed
fi
...
if [ ! -f .extracted ]; then
	...
	tar zxvf perl-5.24.0-cross-1.0.3.tar.gz
	touch .extracted
fi
...
if [ ! -f .extracted ]; then
	rm -rf unrar
	tar zxvf unrarsrc-5.4.2.tar.gz
	touch .extracted
fi

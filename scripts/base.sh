...
if [ ! -f .certs_installed ]; then
	...
	c_rehash .
	touch $SRC/curl/curl-7.55.1/.certs_installed
fi
...
if [ ! -f .extracted ]; then
	rm -rf cd expat-2.2.4
	tar xvjf expat-2.2.4.tar.bz2
	touch .extracted
fi
...
if [ ! -f .extracted ]; then
	...
	tar zxvf libxslt-1.1.30.tar.gz
	touch .extracted
fi
...
if [ ! -f .configured ]; then
	...
	CXXFLAGS=$CXXFLAGS \
	$CONFIGURE \
	--with-libxml-src=$SRC/libxml2/libxml2-2.9.5 \
	--without-python \
	--without-crypto
	touch .configured
fi

...
if [ ! -f .certs_installed ]; then
	...
	c_rehash .
	touch $SRC/curl/curl-7.50.3/.certs_installed
fi
...
if [ ! -f .extracted ]; then
	rm -rf tar zxvf perl-5.24.0
	tar zxvf perl-5.24.0.tar.gz
	tar zxvf perl-cross-1.1.0.tar.gz -C perl-5.24.0 --strip 1
	touch .extracted
fi
...
if [ ! -f .extracted ]; then
	rm -rf unrar
	tar zxvf unrarsrc-5.4.4.tar.gz
	touch .extracted
fi

...
if [ ! -f .certs_installed ]; then
	...
	c_rehash .
	touch $SRC/curl/curl-7.42.1/.certs_installed
fi
...
if [ ! -f .extracted ]; then
	rm -rf sqlite-autoconf-3081001
	tar zxvf sqlite-autoconf-3081001.tar.gz
	touch .extracted
fi
...
if [ ! -f .extracted ]; then
	rm -rf par2cmdline-0.6.12
	tar zxvf par2cmdline-0.6.12.tar.gz
	touch .extracted
fi

cd par2cmdline-0.6.12
...
if [ ! -f .extracted ]; then
	rm -rf unrar
	tar zxvf unrarsrc-5.2.7.tar.gz
	touch .extracted
fi
